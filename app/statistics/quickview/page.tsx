...
export default function QuickViewPage() {
  ...
  const fetchQuickViewData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/statistics/quickview?year=${year}`);
      ...
      if (result.success) {
        // Filter out rows where all data values are null (no data for this year)
  ...
  return (
    <main className='min-h-screen bg-gray-50'>
      <h1>CEAL Statistics Data [Quick view]</h1>
        ...
        <div className='space-y-6'>
          ...
          <div className='bg-amber-100 border border-amber-300 rounded-lg p-4'>
            <p className='text-sm text-amber-900'>
              Data from Institutions in all regions. Year {year}
            </p>
          </div>
                  ...
                  <TableHeader>
                    <TableRow className='bg-teal-700 hover:bg-teal-700'>
                      ...
                      <TableHead
                        rowSpan={2}
                        className='text-white font-semibold border-r border-teal-600 text-center align-middle px-2 py-2 whitespace-normal cursor-pointer select-none'
                        onClick={() => handleSort("other_materials_pct")}
                      >
                        Other Materials Held (%){" "}
                        <SortIcon col='other_materials_pct' />
                      </TableHead>
                      <TableHead
                        rowSpan={2}
                  ...
                  <TableBody>
                    {filteredData.map((row, index) => (
                      ...
                      >
                        ...
                        <TableCell className='text-right border-r border-gray-200 px-2 py-1'>
                          {row.other_materials_pct !== null
                            ? `${row.other_materials_pct.toFixed(2)}%`
                            : "-"}
                        </TableCell>
