...
export default function EBookEditClient({
  libid,
  year,
  rows,
}: {
  ...
  const getColumns = () => [
    ...
    },
    {
      ...
      header: "Subtitle",
      cell: ({ row }: any) => (
        <div className="max-w-[150px] truncate">{row.getValue("subtitle")}</div>
      ),
    },
    {
      ...
      header: "Series #",
      cell: ({ row }: any) => (
        <div className="max-w-[100px] truncate">{row.getValue("sub_series_number")}</div>
      ),
    },
  ];
  ...
  const handleSubscribeSelected = async (selectedRowIds: number[]) => {
    ...
    try {
      ...
      toast.success(`Successfully subscribed to ${result.data.successCount} E-Book record${result.data.successCount === 1 ? "" : "s"}! Redirecting...`);

      // Redirect to subscription management page
      setTimeout(() => {
        router.push(`/admin/forms/${libid}/ebookedit?year=${year}`);
      }, 2000);
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between bg-blue-50 p-4 rounded-lg border border-blue-200">
          <div>
            <h3 className="text-lg font-medium text-blue-900">Add E-Book Subscriptions</h3>
