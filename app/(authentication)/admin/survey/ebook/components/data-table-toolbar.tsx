...
export function DataTableToolbar<TData>({
  ...
  const handleImportPreviousYear = async () => {
    ...
    if (!window.confirm(`Import all ${sourceYear} global and institution-created E-Book entries into ${year}? Existing matching entries will be skipped.`)) return;
    ...
    try {
      const response = await fetch("/api/survey/import-previous-year", {
        ...
        body: JSON.stringify({ resource: "ebook", targetYear: year }),
      });
      const result = await response.json();
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const response = await fetch(`/api/ebook/export-excel/${year}`);
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `EBook_Database_${year}.xlsx`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
