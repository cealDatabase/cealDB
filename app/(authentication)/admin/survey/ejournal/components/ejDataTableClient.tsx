...
export default function EJournalDataTableClient({
  ...
  const [selectionData, setSelectionData] = useState<Map<number, { is_selected: boolean; custom_count: number | null }>>(() => {
    ...
    data.forEach((item) => {
      const withSelection = item as listEJournalWithSelection;
  ...
  const persistOne = useCallback(
    async (
      ...
      if (!libid) return;
      try {
        const response = await fetch("/api/survey/ejournal/save-selections", {
          method: "POST",
  ...
  useEffect(() => {
    if (newRecordId) {
      const timer = setTimeout(() => {
        ...
        router.replace(`/admin/survey/ejournal/${year}`, { scroll: false });
      }, 3000);
