...
export default function ReportsPage() {
  ...
  const [selectedSurveyYear, setSelectedSurveyYear] = useState<string>('');
  const [surveyYears, setSurveyYears] = useState<number[]>([]);
  const [loadingSurvey, setLoadingSurvey] = useState<string | null>(null);
  const [exportedSurveys, setExportedSurveys] = useState<Set<string>>(new Set());
  ...
  const handleExportSurvey = async (reportType: string, reportName: string) => {
    if (!selectedSurveyYear) {
      sonnerToast.error('Please select a year first');
      return;
    }

    setLoadingSurvey(reportType);

    try {
      const response = await fetch(
        `/api/export/supplementary-reports?year=${selectedSurveyYear}&reportType=${reportType}`,
        {
          credentials: 'include',
        }
      );
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `${reportName}-${selectedSurveyYear}.xlsx`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);

      setExportedSurveys(prev => new Set(prev).add(reportType));

      sonnerToast.success(`${reportName} exported successfully`);
    } catch (error) {
      console.error('Export error:', error);
  ...
  const handleExportAllSurveys = async () => {
    ...
    setLoadingSurvey('batch');

    try {
      const response = await fetch(
        `/api/export/supplementary-reports?year=${selectedSurveyYear}&reportType=batch`,
        {
          credentials: 'include',
        }
      );
              ...
              <CardHeader>
                ...
                <CardDescription>
                  {isSuperAdminOrEditor
                    ? "Export cross-year statistics for any institution"
                    : "Export cross-year statistics for your institution"}
                </CardDescription>
              </CardHeader>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <div className="space-y-2 flex-1">
                      <Label htmlFor="survey-year">Select Year</Label>
                      <Select value={selectedSurveyYear} onValueChange={setSelectedSurveyYear}>
                        <SelectTrigger id="survey-year" className="w-[200px]">
                  ...
                  <div className="space-y-3">
                    <Label className="text-base">Export Individual Reports</Label>
                    {SURVEY_REPORTS.map(report => (
                      <div
                        key={report.id}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          {exportedSurveys.has(report.id) && (
                            <CheckCircle className="w-5 h-5 text-green-600" />
                          )}
                          <Button
                            onClick={() => handleExportSurvey(report.id, report.filename)}
                            disabled={!selectedSurveyYear || loadingSurvey !== null}
                            variant="outline"
                            size="sm"
                          >
                            {loadingSurvey === report.id ? (
                              <>
