...
export async function GET(request: NextRequest) {
  try {
    ...
    if (reportType === 'batch') {
      response = await exportBatchReportsWord(yearNum, userCtx);
    } else if (!reportType) {
      return NextResponse.json(
      ...
      response = await exportSingleReportWord(reportType, yearNum, userCtx);
    }

    // Audit log the export
    await logAuditEvent({
      action: 'EXPORT',
      tableName: 'Supplementary Word Report',
