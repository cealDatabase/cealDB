...
import { WordExporter } from '@/lib/wordExporter';
...
// Helper function to calculate fields for each form type (same as Excel export)
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to export Word document', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}

async function exportSingleFormWord(formType: string, year: number) {
  ...
  let multiTierHeaders: any = null;

  // Use same configuration as Excel export
  ...
  fieldMapping = (formFieldMappings as any)[formType];
  groupedHeaders = config.groupedHeaders || [];
  multiTierHeaders = config.multiTierHeaders;

  // Fetch data (same queries as Excel export)
  ...
  data = await prismaQueries[formType]();
  ...
  // Transform data (same logic as Excel export)
  ...
  if (multiTierHeaders) {
    wordConfig.multiTierHeaders = multiTierHeaders;
  } else if (groupedHeaders.length > 0) {
    wordConfig.groupedHeaders = groupedHeaders;
  }
