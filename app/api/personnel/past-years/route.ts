...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch personnel support data for all these library years
    const personnelSupportData = await db.personnel_Support.findMany({
      where: {
        libraryyear: {
    ...
    const dataByYear = pastYears.map(year => {
      ...
      const data = personnelSupportData.find(ps => ps.libraryyear === libraryYearId);
    ...
    console.error("Error fetching past years personnel support data:", error);
    return NextResponse.json(
