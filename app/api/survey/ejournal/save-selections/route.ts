// /app/api/survey/ejournal/save-selections/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    const results = await db.$transaction([
      ...meaningful.map((sel) =>
        db.libraryYear_ListEJournal.upsert({
          where: {
            libraryyear_id_listejournal_id: {
              libraryyear_id: libraryYearId,
              listejournal_id: sel.listId,
            },
          },
          update: {
      ...
      ...(toDelete.length > 0
        ? [
            db.libraryYear_ListEJournal.deleteMany({
              where: {
                libraryyear_id: libraryYearId,
                listejournal_id: { in: toDelete.map((s) => s.listId) },
              },
            }),
          ]
        : []),
    ...
    return NextResponse.json({
      success: true,
      message: `Saved ${results.length} EJournal selections`,
      data: {
        year,
        libraryId,
    ...
    console.error("[EJournal Save Selections] Error:", error);
    ...
    return NextResponse.json(
      { 
        error: "Failed to save EJournal selections",
