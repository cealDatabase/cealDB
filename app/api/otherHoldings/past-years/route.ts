...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch other holdings data for all these library years
    const otherHoldingsData = await db.other_Holdings.findMany({
      where: {
        libraryyear: {
    ...
    const dataByYear = pastYears.map(year => {
      ...
      const data = otherHoldingsData.find(oh => oh.libraryyear === libraryYearId);
    ...
    console.error("Error fetching past years other holdings data:", error);
    return NextResponse.json(
