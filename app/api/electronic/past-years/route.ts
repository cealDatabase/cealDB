...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch electronic data for all these library years
    const electronicData = await db.electronic.findMany({
      where: {
        libraryyear: {
    ...
    const dataByYear = pastYears.map(year => {
      ...
      const data = electronicData.find(e => e.libraryyear === libraryYearId);
    ...
    console.error("Error fetching past years electronic data:", error);
    return NextResponse.json(
