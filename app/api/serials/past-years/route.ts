...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch serials data for all these library years
    const serialsData = await db.serials.findMany({
      where: {
        libraryyear: {
    ...
    const dataByYear = pastYears.map(year => {
      ...
      const data = serialsData.find(s => s.libraryyear === libraryYearId);
    ...
    console.error("Error fetching past years serials data:", error);
    return NextResponse.json(
