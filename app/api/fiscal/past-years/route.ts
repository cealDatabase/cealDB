...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch fiscal support data for all these library years
    const fiscalSupportData = await db.fiscal_Support.findMany({
      where: {
        libraryyear: {
    ...
    const dataByYear = pastYears.map(year => {
      ...
      const data = fiscalSupportData.find(fs => fs.libraryyear === libraryYearId);
    ...
    console.error("Error fetching past years fiscal support data:", error);
    return NextResponse.json(
