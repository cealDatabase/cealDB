...
export async function POST(req: Request) {
  try {
    ...
    console.log("Unsubscribing from E-Book records with body:", body);
    ...
    // Remove subscription entries from LibraryYear_ListEBook junction table
    ...
    for (const recordId of validRecordIds) {
      try {
        ...
        if (!existingSubscription) {
          ...
          // Remove subscription
          await db.libraryYear_ListEBook.delete({
            where: {
              libraryyear_id_listebook_id: {
                libraryyear_id: libraryYearId,
                listebook_id: Number(recordId),
    ...
    console.error("API error (unsubscribe E-Book):", error);

    return NextResponse.json(
      { error: "Failed to unsubscribe from E-Book records", detail: error?.message },
      { status: 500 }
    );
  }
}
