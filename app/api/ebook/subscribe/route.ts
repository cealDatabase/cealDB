...
export async function POST(req: Request) {
  try {
    ...
    console.log("Subscribing to E-Book records with body:", body);
    ...
    const existingRecords = await db.list_EBook.findMany({
      where: {
    ...
    // Create subscription entries using LibraryYear_ListEBook junction table
    ...
    for (const recordId of validRecordIds) {
      try {
        ...
        const existingSubscription = await db.libraryYear_ListEBook.findUnique({
          where: {
        ...
        if (existingSubscription) {
          ...
          const subscription = await db.libraryYear_ListEBook.create({
            data: {
              libraryyear_id: libraryYearId,
              listebook_id: Number(recordId),
            },
    ...
    console.error("API error (subscribe E-Book):", error);

    return NextResponse.json(
      { error: "Failed to subscribe to E-Book records", detail: error?.message },
      { status: 500 }
    );
  }
}
