...
export async function POST(req: Request) {
  try {
    ...
    // Remove subscription entries from LibraryYear_ListEJournal junction table
    ...
    for (const recordId of validRecordIds) {
      try {
        ...
        if (!existingSubscription) {
          ...
          // Remove subscription
          await db.libraryYear_ListEJournal.delete({
            where: {
              libraryyear_id_listejournal_id: {
                libraryyear_id: libraryYearId,
                listejournal_id: Number(recordId),
    ...
    console.error("API error (unsubscribe E-Journal):", error);

    return NextResponse.json(
      { error: "Failed to unsubscribe from E-Journal records", detail: error?.message },
      { status: 500 }
    );
  }
}
