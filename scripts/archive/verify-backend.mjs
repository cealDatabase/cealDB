#!/usr/bin/env node
/**
 * Backend verification script (ES Module version)
 * Run with: node scripts/verify-backend.mjs
 */

// Use dynamic import for PrismaClient from the generated location
const { PrismaClient } = await import('../prisma/generated/client/client.ts').catch(() => {
  // Fallback: try importing from the standard location
...
async function verifyTestData() {
  ...
  try {
    ...
    const testData = await prisma.libraryYear_ListAV.findMany({
    ...
    console.log(`✅ Found ${testData.length} test records for libraryYear 2816`);
    testData.forEach(record => {
      console.log(`   listav_id=${record.listav_id}, is_selected=${record.is_selected}, custom_count=${record.custom_count}`);
    });
